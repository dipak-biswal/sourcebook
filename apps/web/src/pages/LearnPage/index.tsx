  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      ...
      <ul className="min-h-0 flex-1 overflow-y-auto p-2">
        {catalog.topics.map((t: LearnTopic) => {
          ...
          return (
            <li key={t.id}>
              <button
                ...
                className={cn(
                  "mb-0.5 w-full rounded-[8px] px-2.5 py-2 text-left transition-colors",
                  active
                ...
                {t.summary && (
                  <div
                    className={cn(
                      ...
                      active ? "text-[var(--canvas)]/75" : "text-mute",
                    )}
                  >
                    {t.summary}
                  </div>
                  ...
                  <div className="mt-1 flex flex-wrap gap-1">
                    {t.tags.slice(0, 3).map((tag) => (
                      <span
                        key={tag}
                        className={cn(
                          "rounded-[4px] px-1 py-px text-[9px] uppercase tracking-wide",
                          active
                            ? "bg-[var(--canvas)]/15 text-[var(--canvas)]/90"
      ...
      {lesson && (
        <div className="max-h-[42%] shrink-0 overflow-y-auto border-t border-hairline">
...
function LearnPageInner() {
  ...
  useEffect(() => {
    if (!catalog?.topics?.length) return;
    if (
      selectedTopicId &&
