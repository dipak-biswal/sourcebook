...
from app.agents.visual_summary.handoff.structured import format_plan_layout_prompt
from app.agents.visual_summary.workspace.context import (
    derive_workspace_context,
    packet_from_dict,
    resolve_workspace_context,
)
from app.agents.visual_summary.workspace.profile import (
    context_fingerprint,
...
def _mock_profiler(monkeypatch, payload: dict, calls: list | None = None):
    ...
    monkeypatch.setattr(
        "app.agents.visual_summary.workspace.profile._client", lambda: fake_client
    )
    monkeypatch.setattr(
        "app.agents.visual_summary.workspace.profile.settings.workspace_llm_profiler", True
    )
    monkeypatch.setattr(
        "app.agents.visual_summary.workspace.profile.settings.openai_api_key", "sk-test"
    )
...
def test_resolve_recomputes_budgets_from_fresh_evidence(monkeypatch, db_session):
    ...
    import app.agents.visual_summary.workspace.profile as profile_mod
